...
class UserController extends Controller
{
    ...
    {
        $this->validate($request, [
            'email' => 'required|email|exists:Userlogin,userlogin',
            //'password' => 'required|string|min:6'
        ]);
        $user = Userlogin::where('userlogin', $request->email)->where('userpassword', md5($request->password))->first();
        if ($user) {
            $token = Str::random(40);
